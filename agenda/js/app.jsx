...
function App() {
  ...
  const applyServerConfig = React.useCallback((settings, cfg) => {
    ...
    next.extraPageEnabled = !!cfg.extra_page_enabled;
    next.extraPageTitle = cfg.extra_page_title || '';
    return next;
  }, []);
  ...
  const bookingKey = state ? JSON.stringify({
    ...
    xe: state.settings.extraPageEnabled, xt: state.settings.extraPageTitle,
  }) : '';
  ...
  else if (tab === 'settings') screen = <SettingsScreen c={c} state={state} onBack={backHome} onUpdateSettings={updateSettings} onSyncNow={pushAll} syncStatus={syncStatus} onLogout={handleLogout} onOpenAlbum={() => setTab('album')} onOpenExtraPage={() => setTab('extraPage')} />;
  else if (tab === 'album') screen = <AlbumScreen c={c} state={state} onBack={() => setTab('settings')} onSessionExpired={handleLogout} />;
  else if (tab === 'extraPage') screen = <ExtraPageScreen c={c} state={state} onBack={() => setTab('settings')} onSessionExpired={handleLogout} />;
  ...
  const hiddenTabs = ['detail', 'flagged', 'settings', 'album', 'extraPage', 'inbox', 'new'];
