...
function ExtraPageEditor({ c, s, setS, T, onOpenExtraPage }) {
  ...
  return (
    <div>
      <div style={{ fontSize: 11, color: c.muted, marginBottom: 12, lineHeight: 1.5 }}>{T.extraPageHint}</div>
      ...
      <div style={{
        ...
        <div>
          <div style={{ fontSize: 13, color: c.ink, fontWeight: 500 }}>{T.extraPageEnabled}</div>
          <div style={{ fontSize: 11, color: c.muted, marginTop: 3 }}>{T.extraPageEnabledHint}</div>
        </div>
        <Toggle c={c} on={enabled} onClick={() => setS({ extraPageEnabled: !enabled })} />
      </div>

      <FieldBlock label={T.extraPageTitle}>
        <input type="text" maxLength={40} value={title} placeholder={T.extraPageTitlePlaceholder}
          onChange={e => setS({ extraPageTitle: e.target.value.slice(0, 40) })}
          style={inp(c, FONTS.ui, 14)} />
